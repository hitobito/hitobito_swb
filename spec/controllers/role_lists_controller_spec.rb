...
describe RoleListsController do
  ...
  describe "POST#create" do
    ...
    it "allows creation of normal role" do
      expect do
        post :create, params: {group_id: group.id, ids: person.id, role: {type: Group::Verein::JSCoach.sti_name, group_id: group.id}}
    ...
    it "prevents creation of ts managed role" do
      expect do
        post :create, params: {group_id: group.id, ids: person.id, role: {type: Group::Verein::Interclub.sti_name, group_id: group.id}}
  ...
  describe "PUT#update" do
    it "allows changing from normal to normal type" do
      ...
      end.to change { role.person.reload.roles.last.type }.from(role.type).to("Group::Verein::JSCoach")
    ...
    it "prevents changing ts managed role" do
      role = Fabricate(Group::Verein::Interclub.sti_name, group: group, ts_code: Faker::Internet.uuid)
      expect do
        put :update, params: {
          group_id: group.id,
          ids: role.person_id,
  ...
  describe "DELETE#destroy" do
    it "allows deletion normal role" do
      role = Fabricate(Group::Verein::Ausbildung.sti_name, group: group)
      expect do
        delete :destroy, params: {group_id: group.id, ids: role.person_id, role: {types: {"Group::Verein::Ausbildung" => "1"}}}
      end.to change { group.roles.count }.by(-1)
    ...
    it "prevents deletion of ts managed role" do
      ...
      expect do
        delete :destroy, params: {group_id: group.id, ids: role.person_id, role: {types: {"Group::Verein::Interclub" => "1"}}}
      end.not_to change { group.roles.count }
