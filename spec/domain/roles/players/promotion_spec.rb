...
describe Roles::Players::Promotion do
  ...
  context "JuniorU15" do
    ...
    it "promotes multiple types of JuniorU15 roles" do
      ...
      expect(region_spieler.roles.first.type).to eq "Group::RegionSpieler::Aktivmitglied"
  ...
  context "JuniorU19" do
    ...
    it "noops if person is within limit no exceeded limit" do
      person.update_columns(birthday: 18.years.ago.end_of_year)
      expect do
        promotion.run
      end.not_to change { person.reload.roles.map(&:type) }
    end
    ...
    it "promotes to Aktivmitglied if outside of U19 year range" do
      person.update_columns(birthday: 19.years.ago.beginning_of_year)
      expect do
        promotion.run
      end.not_to change { person.reload.roles.map(&:type) }
    end
    ...
    it "promotes multiple types of JuniorU19 roles" do
      ...
      expect(thun_spieler.roles.first.type).to eq "Group::VereinSpieler::Aktivmitglied"
      expect(region_spieler.roles.first.type).to eq "Group::RegionSpieler::Aktivmitglied"
      expect(root_spieler.roles.first.type).to eq "Group::DachverbandSpieler::Aktivmitglied"
    end
  end
end
