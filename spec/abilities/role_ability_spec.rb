...
describe RoleAbility do
  ...
  it "calls phase_checker during create" do
    expect_any_instance_of(Roles::Players::PhaseChecker).to receive(:create?).once.and_return(true)
  ...
  it "calls phase_checker during update" do
    expect_any_instance_of(Roles::Players::PhaseChecker).to receive(:update?).once.and_return(true)
    expect(ability).to be_able_to(:update, player_role)
  end

  it "calls phase_checker during destroy" do
    expect_any_instance_of(Roles::Players::PhaseChecker).to receive(:destroy?).once.and_return(true)
