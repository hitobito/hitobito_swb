  ...
  class Promotion
    ...
    def promote(role)
      Role.transaction do
        ...
        create_log_entry(new_role, level: :info, message: "Promoted #{role} to #{new_role} for (#{role.person})")
      end
    rescue => exception
      create_log_entry(role, level: :error, message: "Failed to promote #{role} for (#{role.person})")
